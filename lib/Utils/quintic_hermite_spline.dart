...
class QuinticHermiteSpline {
  ...
  Vectors getVectors(double time){
    ...
    double adjustedTime = (time - vectors[segmentIdx].time)/(vectors[segmentIdx+1].time - vectors[segmentIdx].time);// Making a time between 0 and 1
    ...
    Matrix velocity = Matrix.fromList([
      List.generate(6, (int index){
        return index == 0 ? 0 : index*pow(adjustedTime, index-1) as double;
      }),
    ])*segmentCoefficients[segmentIdx];
    Matrix acceleration = Matrix.fromList([
      List.generate(6, (int index){
        return index <= 1 ? 0 : index*(index-1)*pow(adjustedTime, index-2) as double;
      }),
    ])*segmentCoefficients[segmentIdx];
    return Vectors(position: position[0][0], velocity: velocity[0][0], acceleration: acceleration[0][0], time: time);
  }
}
